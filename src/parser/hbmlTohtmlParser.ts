// Retrieved from & Credit to: https://github.com/heyitsdoodler/hbml/blob/main/parser.js
/*
 ...
 */
const TRUE_ROOT: string = "__hbml_true_root__";

const createIterString = (string: string): any => {
  let index = 0;
...
const convertReservedChar = (string: string): string => {
...
// Object is defined in line 15, will refactor later
const parseComment = (iter: any): string => {
  let outputComment = "";
...
const parseStr = (iter: any): string => {
  let outputStr = "";
...
const parseSelector = (
  ...
  const iter = createIterString(
    selector.trim().replace(/^:root/, 'html[lang="en"]')
  );
  ...
  return {
    ...
    attributes: convertReservedChar(
      `${joinedIds.length ? `id="${joinedIds}" ` : ""}${
        joinedClasses.length ? `class="${joinedClasses}" ` : ""
      }${attributes}`.trim()
    ),
  };
};
...
const parseElChild = (iter: any, parentTag = ""): string => {
  const next = iter.next();
...
// Parse HBML element
const parseEl = (iter: any, parentTag = ""): string => {
  let unparsedSelector = "";
  let childrenOutputs: string[] = [];
  ...
  while (iter.hasNext() && mode <= 1) {
    ...
    switch (mode) {
      case 0:
        ...
        break;
      case 1:
        const { tag } = parseSelector(
          unparsedSelector,
          INLINE_ELEMENTS.includes(parentTag.toLowerCase())
        );
        ...
        if (next === "}" && multiline) mode = 2;
          ...
          if (!VOID_ELEMENTS.includes(tag.toLowerCase())) {
            ...
            childrenOutputs = [
              `<!-- Void element ${tag} may not have child nodes -->`,
            ];
        }
        break;
    }
  ...
  const { tag, attributes } = parseSelector(
    unparsedSelector,
    INLINE_ELEMENTS.includes(parentTag.toLowerCase())
  );
  ...
  return `${tag.toLowerCase() === "html" ? `<!doctype html>` : ""}<${tag}${
    attributes ? ` ${attributes}` : ""
  }${!closingTag ? `/` : ""}>${childrenOutputs.join("")}${
    closingTag ? `</${tag}>` : ""
  }`;
};

export const parseHBML = (hbml: string) => {
  // The input is wrapped in the "TRUE_ROOT" you can have multiple top level components, such as comments
  const parsed = `${parseElChild(
    createIterString(`${TRUE_ROOT}{${hbml.trim()}}`)
  )}`;

  // The "TRUE_ROOT" is stripped out of the final HTML output
  return parsed.substring(
    TRUE_ROOT.length + 2,
    parsed.length - (TRUE_ROOT.length + 3)
  );
};
